function feed() {
var element;
    if (flower.health !==0) {
        if (flower.satiety < 50) {
            flower.satiety += 5;
            flower.health += 2;

            if (flower.health < 50) {
                flower.power += 2;
                document.querySelector('.health-count').innerHTML = flower.health;
            }

            if (flower.power < 10) {
                flower.power += 2;
                document.querySelector('.health-count').innerHTML = flower.power;
            }

            document.querySelector('.satiety-count').innerHTML = flower.satiety;

        } else {
            element = document.createElement('div');
            element.innerHTML = 'Flower is not hungry';
            document.querySelector('.flower').appendChild(element);
        }
    }
}

function treat() {
    var element;
    if (flower.health !== 0) {
        if (flower.health < 99) {
            flower.health += 2;
            document.querySelector('.health-count').innerHTML = flower.health;

        } else {
            element = document.createElement('div');
            element.innerHTML = 'Flower is healthy';
            document.querySelector('.flower').appendChild(element);
        }
    }
}

function play() {
    if (flower.health !== 0) {
        if (flower.happiness < 50) {
            flower.happiness += 5;
            flower.power--;
            document.querySelector('.health-count').innerHTML = flower.health;
            document.querySelector('.power-count').innerHTML = flower.power;
        }
    }
}

function sleep() {
    var element;
    if (flower.health !== 0) {
        if (flower.weariness < 50) {
            flower.weariness += 5;
            if (flower.power<10) {
                flower.power++;
                document.querySelector('.power-count').innerHTML = flower.power;
            }
            document.querySelector('.weariness-count').innerHTML = flower.weariness;
        } else {
            element = document.createElement('div');
            element.innerHTML = 'Flower is not tired';
            document.querySelector('.flower').appendChild(element);
        }
    }
}
